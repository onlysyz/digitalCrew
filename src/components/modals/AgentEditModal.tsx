...
export default function AgentEditModal({ isOpen, agent, onClose, onSubmit, models }: AgentEditModalProps) {
  ...
  // Populate fields when agent changes
  useEffect(() => {
    if (agent) {
      setName(agent.name || '');
      setDescription(agent.description || '');
      setRole(agent.role || 'worker');
      setModelName(agent.llm_config?.model_name || '');
      setSystemPrompt(agent.system_prompt || '');
      setCapabilities(agent.capabilities || []);
      // Initialize tool permissions based on agent's existing permissions
      const agentToolNames = agent.tool_permissions?.map(t => t.tool_name) || [];
      setToolPermissions(
        TOOL_OPTIONS.map(t => ({
          tool_name: t.tool_name,
          enabled: agentToolNames.includes(t.tool_name)
        }))
      );
    }
  }, [agent]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-surface-container-highest border border-white/10 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/5 sticky top-0 bg-surface-container-highest">
          <h2 className="font-display text-xl font-bold text-on-surface">编辑代理配置</h2>
